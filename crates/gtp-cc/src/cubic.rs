...
impl CongestionController for CubicCongestionController {
    ...
    fn on_ack(&mut self, ack_event: &AckEvent, now: MonotonicTime) {
        ...
        // FR-4: the RTT the controller uses for pacing and the once-per-RTT loss guard
        // is synced from the loss detector (the single source of truth) via `on_rtt`,
        // called by the connection after each ACK — the controller no longer derives a
        // second, independent RTT estimate from the raw sample here.
