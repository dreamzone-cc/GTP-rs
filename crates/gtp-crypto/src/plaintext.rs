...
mod tests {
    ...
    fn test_plaintext_protector_passthrough() {
        ...
        let sealed_len = protector.seal(PacketNumber(1), ConnectionId(10), b"aad", &mut buf, 5).unwrap();
        assert_eq!(sealed_len, 5);

        let opened_len = protector.open(PacketNumber(1), ConnectionId(10), b"aad", &mut buf).unwrap();
